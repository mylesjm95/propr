...
export async function POST() {
  ...
  // Initialize stats object early to avoid reference errors
  const stats = {
    usersWithSearches: 0,
    searchesProcessed: 0,
    listingsFound: 0,
    ...
    failedQueries: [],
    emailErrors: [],
    timestamp: new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString().replace(/\.\d{3}Z$/, 'Z')
  };
  ...
  // Timestamp is now initialized at the beginning of the function
  ...
  for (const user of usersWithSearches) {
    ...
    for (const search of searches) {
      ...
      try {
        ...
        // Create the filter using the street number and name exactly matching the working query format
        const filter = `StreetNumber eq '${sanitizedStreetNumber}' and contains(StreetName, '${sanitizedStreetName}') and ModificationTimestamp ge ${stats.timestamp}`;
    ...
    const emailBody = `
      ...
      <p>New listings for your saved condo buildings from the last 24 hours:</p>
      <ul>${listingsHtml}</ul>
    ...
    try {
      const emailResponse = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          to: user.email,
          subject: `Daily Listings Update for ${dateStr} - ${allListings.length} New Listings`,
          html: emailBody,
        }),
      });
  ...
  // Update the stats with key metrics
