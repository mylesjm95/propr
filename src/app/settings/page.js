...
export default async function Settings() {
  ...
  // Fetch the user's saved searches from the SavedSearch table
  const { getUserSavedSearches } = await import('@/lib/db');
        ...
        <Tabs defaultValue="profile" className="w-full">
          ...
          <TabsContent value="saved-searches" className="space-y-8">
            <div className="bg-card rounded-lg shadow p-6">
              ...
              <SavedSearches user={authenticatedUser} savedSearches={savedSearches || []} />
            </div>
          </TabsContent>
        </Tabs>
      </div>
